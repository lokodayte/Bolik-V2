...
// Depends on shared.js being loaded first.
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // being already signed in on page load. The domain check and
            // the "redirect once a profile exists" check apply to every
            // sign-in method. Password-based flows (login/registration/
            // migration) already create their own profile via an explicit
            // call before redirecting, so there's nothing left to do here
            ...
            _auth.onAuthStateChanged(user => {
                ...
                _ref.users.child(user.uid).once('value').then(snap => {
                    if (snap.val()) window.location.href = 'index.html';
                });
            });
